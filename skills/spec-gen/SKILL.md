...
| **ER / データモデル** | `docs/architecture/er.md` | 全体ER図、テーブル別ER図、テーブル定義、制約、インデックス方針 |
...
1. **全体ER図**: すべてのテーブルとリレーションを1つの `erDiagram` で俯瞰する。カラムは含めず、テーブル名とリレーションのみで全体像を示す
2. **テーブル別ER図**: テーブルごとに個別の `erDiagram` を作成し、カラム名・型・制約（PK, FK, NOT NULL 等）を詳細に記述する。関連テーブルとのリレーションも含める
