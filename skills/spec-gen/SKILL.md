...
- プロジェクトの慣習があればそれに従う。既存仕様書がある場合は既存構成に合わせる
...
`AskUserQuestion` で確認：
- **GitHub Issue + ブランチ作成**（推奨）: feat Issue を作成し作業ブランチで作業。impl スキルとの連携に最適
...
3. ドキュメント範囲を合意
4. **GitHub Issue モード**: ベースブランチ記録 → feat Issue 作成 → `feat/#<Issue番号>` ブランチ作成 → Issue にブランチ名追記
   - **Issue タイトルは「実装する機能」を記述する**（例: `feat: ユーザー認証機能`）。「設計ドキュメント作成」のようなドキュメント作業自体をタイトルにしない
   - Issue 本文は実装要件のサマリーとし、設計ドキュメントへのリンクは補足として追記する
   - これにより `impl` スキルがこの Issue を読んだとき、正しく機能実装に取り組める
5. TaskCreate でタスク化

#### 既存仕様書への追記の場合
1. 既存仕様書を読み込み現在の仕様を把握
2. 追加内容をヒアリング
3. 既存機能の再利用・変更案を提示
4. 方針を合意
5. **GitHub Issue モード**: 上記と同様（Issue タイトルは追加する機能を記述する）
6. TaskCreate でタスク化
...
3. **GitHub Issue モード**: push → Issue に仕様書リンク追記 → サマリー報告（Issue URL 含む）
