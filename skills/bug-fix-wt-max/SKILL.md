...
`bug-fix-max` の worktree 隔離版。メインの作業ツリーを汚さずにバグ修正を行う。
...
- **Issue 番号** (例: `/bug-fix-wt-max #42`): GitHub Issue からバグ報告を取得
- **Issue URL** (例: `/bug-fix-wt-max https://github.com/owner/repo/issues/42`): 同上
- **テキスト** (例: `/bug-fix-wt-max ログイン時に500エラー`): テキストをバグ報告として扱う
